<?php
// auto-generated by sfPropelCrud
// date: 2010/12/01 01:27:40
?>
<h1>Actividades poa a partir de procedimientos</h1>

<table class="data">
<thead>
<tr>
  <th>Actividad</th>
  <th>Meta poa</th>
  <th>Proceso</th>
  <th>Tipo</th>
  <th>Procedimiento</th>
  <th>Proyecto</th>
  <th>Responsable</th>
  <th>Observaciones</th>
</tr>
</thead>
<tbody>
<?php foreach ($actividad_poas as $actividad_poa): ?>
<tr>
    <td><?php echo link_to($actividad_poa->getDescripcion(), 'actividadpoaprocedimiento/show?id='.$actividad_poa->getId()) ?></td>
      <td><?php echo $actividad_poa->getMetaPoa() ?></td>
      <td><?php echo $actividad_poa->getProceso() ?></td>
      <td><?php echo $actividad_poa->getTipo() ?></td>
      <td><?php echo $actividad_poa->getProcedimiento() ?></td>
      <td><?php echo $actividad_poa->getProyectoId() ?></td>
      <td><?php echo $actividad_poa->getResponsable() ?></td>
      <td><?php echo $actividad_poa->getObservaciones() ?></td>
  </tr>
<?php endforeach; ?>
</tbody>
</table>
