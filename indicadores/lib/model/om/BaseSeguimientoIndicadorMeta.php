...
abstract class BaseSeguimientoIndicadorMeta extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SeguimientoIndicadorMetaPeer::ID;
		}
	...
	public function setIndicadorMetaId($v)
	{
		...
		if ($this->indicador_meta_id !== $v) {
			...
			$this->modifiedColumns[] = SeguimientoIndicadorMetaPeer::INDICADOR_META_ID;
		}
	...
	public function setAnyo($v)
	{
		...
		if ($this->anyo !== $v) {
			$this->anyo = $v;
			$this->modifiedColumns[] = SeguimientoIndicadorMetaPeer::ANYO;
		}
	...
	public function setValor($v)
	{
		...
		if ($this->valor !== $v) {
			$this->valor = $v;
			$this->modifiedColumns[] = SeguimientoIndicadorMetaPeer::VALOR;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			throw new PropelException("Error populating SeguimientoIndicadorMeta object", $e);
		}
	}
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			SeguimientoIndicadorMetaPeer::doDelete($this, $con);
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->aIndicadorMeta !== null) {
				...
				$this->setIndicadorMeta($this->aIndicadorMeta);
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = SeguimientoIndicadorMetaPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += SeguimientoIndicadorMetaPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SeguimientoIndicadorMetaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SeguimientoIndicadorMetaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = SeguimientoIndicadorMetaPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SeguimientoIndicadorMetaPeer::ID)) $criteria->add(SeguimientoIndicadorMetaPeer::ID, $this->id);
		if ($this->isColumnModified(SeguimientoIndicadorMetaPeer::INDICADOR_META_ID)) $criteria->add(SeguimientoIndicadorMetaPeer::INDICADOR_META_ID, $this->indicador_meta_id);
		if ($this->isColumnModified(SeguimientoIndicadorMetaPeer::ANYO)) $criteria->add(SeguimientoIndicadorMetaPeer::ANYO, $this->anyo);
		if ($this->isColumnModified(SeguimientoIndicadorMetaPeer::VALOR)) $criteria->add(SeguimientoIndicadorMetaPeer::VALOR, $this->valor);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(SeguimientoIndicadorMetaPeer::ID, $this->id);
	...
	{
		if (self::$peer === null) {
			self::$peer = new SeguimientoIndicadorMetaPeer();
		}
		return self::$peer;
	}
