...
/**
 * Subclass for representing a row from the 'subactividad_procedimiento_poa' table.
 *
 * 
 *
 * @package lib.model
 */ 
class SubactividadProcedimientoPoa extends BaseSubactividadProcedimientoPoa
{
  ...
  /**
   * Obtiene el acumulado de la ejecucion de la tabla subactividad_poa_ejecucion
   */
  public function getEjecucion()
  {
    $c = new Criteria();
    $c->add(SubactividadProcedimientoPoaEjecucionPeer::SUBACTIVIDAD_POA_ID, $this->getId());
    $resultset = SubactividadProcedimientoPoaEjecucionPeer::doSelect($c);
  ...
  /**
   * Calcula la Ejecucion ponderada de la subactividad poa como el avance de la subactividad
  ...
  /**
   * Calcula el numero de la semana en la cual se ha realizado la ejecucion de la subactividad
   ...
   * subactividad poa, calculada a partir de la fecha de inicio y la duracion
   *
